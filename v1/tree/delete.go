...
func (t *Node) Delete(interval interval.IInterval) (out *Node, err error) {
	defer Return(&err)
	t.mu.Lock()
	defer t.mu.Unlock()
...
func (t *Node) RemoveRange(start, end time.Time) (out *Node, removed []interval.IInterval) {
	t.mu.Lock()
	defer t.mu.Unlock()
	...
	freed := make([]interval.IInterval, len(nodes))
	for i, n := range nodes {
		freed[i] = t.free(n)
	}
